...
(async () => {
  ...
  if (issues.importAudit.length === 0) {
    ...
    issues.importAudit.forEach(issue => {
      console.log(
        `   ${issue.file}:${issue.line} - '${issue.missingExport}' not exported from '${issue.importPath}'`
      );
    });
  }
  ...
  if (issues.pathResolution.length === 0) {
    ...
    issues.pathResolution.forEach(issue => {
      console.log(
        `   ${issue.file}:${issue.line} - '${issue.importPath}' does not resolve to existing file (tried ${issue.resolvedPath})`
      );
    });
  }
  ...
  const totalIssues =
    issues.importAudit.length + issues.pathResolution.length + issues.circularDeps.length;
