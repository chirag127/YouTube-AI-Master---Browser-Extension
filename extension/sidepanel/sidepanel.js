...
import { _id, $$, _on, lg, l, w, e } from '../utils/shortcuts.js';
const ss = new StorageService(), cs = new ChunkingService();
let gs = null, scs = null, ctx = '', segs = [];
const ab = _id('analyze-btn'), st = _id('status'), aw = _id('auth-warning'), tbs = $$('.tab-btn'), tcs = $$('.tab-content'), sc = _id('summary-content'), ic = _id('insights-content'), tc = _id('transcript-container'), ci = _id('chat-input'), csb = _id('chat-send-btn'), ch = _id('chat-history');
_on(document, 'DOMContentLoaded', async () => {
    ...
    gs = new GeminiService(geminiApiKey); scs = new SegmentClassificationService(gs, cs);
    try { await gs.fetchAvailableModels(); } catch (x) { }
    for (const b of tbs) {
        _on(b, 'click', () => {
    ...
    _on(ci, 'keypress', e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleChat(); } });
...
async function handleChat() {
    ...
    appendMsg('user', q); ci.value = '';
    const lid = appendMsg('ai', 'Thinking...');
    try { const a = await gs.chatWithVideo(q, ctx); await updateMsg(lid, a); }
    catch (x) { await updateMsg(lid, `Error: ${x.message}`); }
}
function appendMsg(r, t) {
    const d = document.createElement('div');
    d.className = `chat-message ${r}`; d.id = `msg-${Date.now()}`; d.textContent = t;
    ch.appendChild(d); ch.scrollTop = ch.scrollHeight; return d.id;
}
async function updateMsg(id, t) {
    const d = _id(id);
    if (d) { d.innerHTML = await parseMarkdown(t); ch.scrollTop = ch.scrollHeight; }
}
_on(ab, 'click', () => analyzeVideo());
async function analyzeVideo(retryCount = 0) {
    const maxRetries = 2;
    try {
        setStatus('loading', 'Fetching video info...'); ab.disabled = true; showLoadingState();
        ...
        try {
            ...
            md = mr.metadata;
        } catch (x) { w('Metadata fetch failed, using fallback:', x); md = { title: 'Unknown Title', author: 'Unknown Channel', videoId: v }; }
        setStatus('loading', 'Fetching transcript...');
        const tr = await sendMessageWithRetry(tab.id, { action: 'GET_TRANSCRIPT', videoId: v }, 2);
        if (tr.error) {
            if (tr.error.includes('does not have captions')) showError('No Captions Available', 'This video does not have captions/subtitles. Please try a different video that has closed captions enabled.');
            else showError('Transcript Error', tr.error);
            return;
        }
        const ts = tr.transcript;
        if (!ts || ts.length === 0) throw new Error('Transcript is empty. Please try a different video.');
        ...
        try {
            ...
            segs = cls; renderTranscript(cls);
            chrome.tabs.sendMessage(tab.id, { action: 'SHOW_SEGMENTS', segments: cls }).catch(x => w('Failed to send segments:', x));
        } catch (x) { w('Segment classification failed:', x); renderTranscript(ts.map(t => ({ ...t, label: null }))); }
        ...
        const cr = await chrome.tabs.sendMessage(tab.id, { action: 'GET_COMMENTS' }).catch(x => { e('Failed to get comments:', x); return { comments: [] }; });
        ...
        if (cms.length > 0) {
            try { l('[Sidepanel] Sending comments to Gemini:', cms); ca = await gs.analyzeCommentSentiment(cms); l('[Sidepanel] Comment analysis result:', ca); }
            catch (x) { e('Comment analysis failed:', x); ca = `Failed to analyze comments: ${x.message}`; }
        ...
        try { await ss.saveTranscript(v, md, ts, an.summary); } catch (x) { w('Failed to save to history:', x); }
    } catch (x) {
        e('Analysis error:', x);
...
async function sendMessageWithRetry(tabId, message, maxRetries = 3) {
    for (let i = 0; i < maxRetries; i++) {
        ...
        catch (x) { if (i === maxRetries - 1) throw x; await new Promise(r => setTimeout(r, 500 * (i + 1))); }
    }
}
...
function showError(title, message) {
    ...
    sc.innerHTML = errorHtml; setStatus('error', `Error: ${title}`);
}
...
function renderTranscript(sg) {
    ...
    for (const s of sg) {
        ...
        tm.className = 'timestamp'; tm.textContent = fmtTime(s.start);
        const tx = document.createElement('span');
        tx.className = 'text'; tx.textContent = s.text;
        if (s.label) {
            const lb = document.createElement('span');
            lb.className = 'segment-label'; lb.textContent = s.label; lb.title = getSgDesc(s.label);
            d.appendChild(lb);
        }
        d.appendChild(tm); d.appendChild(tx);
        _on(d, 'click', () => seekVideo(s.start));
        tc.appendChild(d);
    }
}
function getSgClass(l) {
    const m = { Sponsor: 'segment-sponsor', 'Interaction Reminder': 'segment-interaction', 'Self Promotion': 'segment-self-promo', 'Unpaid Promotion': 'segment-unpaid-promo', Highlight: 'segment-highlight', 'Preview/Recap': 'segment-preview', 'Hook/Greetings': 'segment-hook', 'Tangents/Jokes': 'segment-tangent', Content: 'segment-content' };
    return m[l] || 'segment-unknown';
}
function getSgDesc(l) {
    const d = { Sponsor: 'Paid advertisement or sponsorship', 'Interaction Reminder': 'Asking viewers to like/subscribe/share', 'Self Promotion': 'Promoting creator\'s own products/services', 'Unpaid Promotion': 'Shout-outs to other creators/channels', Highlight: 'Most important or interesting part', 'Preview/Recap': 'Coming up next or previously on', 'Hook/Greetings': 'Video introduction or greeting', 'Tangents/Jokes': 'Off-topic content or humor', Content: 'Main video content' };
    return d[l] || 'Unknown segment type';
}
async function seekVideo(sec) {
    try {
        const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
        if (tab?.id) await chrome.tabs.sendMessage(tab.id, { action: 'SEEK_TO', timestamp: sec });
    } catch (x) { }
}
function fmtTime(s) {
