export class UIAppearance {
  ...
  attachListeners() {
    try {
      ...
      if (ff)
        ff?.addEventListener('change', e => this.handleChange('ui.fontFamily', e.target.value));
      if (is)
        is?.addEventListener('change', e => this.handleChange('ui.iconStyle', e.target.value));
      if (wm)
        wm?.addEventListener('change', e =>
          this.handleChange('ui.wallpaperMode', e.target.checked)
        );
      if (cm)
        cm?.addEventListener('change', e => this.handleChange('ui.compactMode', e.target.checked));
      if (pc)
        pc?.addEventListener('change', e => this.handleChange('ui.primaryColor', e.target.value));
      if (ac)
        ac?.addEventListener('change', e => this.handleChange('ui.accentColor', e.target.value));
      if (bc)
        bc?.addEventListener('change', e =>
          this.handleChange('ui.backgroundColor', e.target.value)
        );
      if (tc)
        tc?.addEventListener('change', e => this.handleChange('ui.textColor', e.target.value));
      if (boc)
        boc?.addEventListener('change', e => this.handleChange('ui.borderColor', e.target.value));
      if (gb) {
      ...
      if (as) {
        ...
        as?.addEventListener('change', e =>
          this.handleChange('ui.animationSpeed', +e.target.value)
        );
      }
      if (ec)
        ec?.addEventListener('change', e => this.handleChange('ui.easingCurve', e.target.value));
      if (mi)
        mi?.addEventListener('change', e =>
          this.handleChange('ui.microInteractions', e.target.checked)
        );
      if (bw) {
      ...
      if (si) {
        ...
        si?.addEventListener('change', e =>
          this.handleChange('ui.shadowIntensity', +e.target.value)
        );
      }
      if (gi) {
      ...
      if (ga)
        ga?.addEventListener('change', e =>
          this.handleChange('ui.gradientAccents', e.target.checked)
        );
    } catch (err) {
