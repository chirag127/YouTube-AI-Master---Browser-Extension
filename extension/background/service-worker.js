...
async function handleFetchPipedMetadata(request, sendResponse) {
  ...
  for (let i = 0; i < Math.min(instances.length, 5); i++) {
    ...
    try {
      ...
      console.log(`[Piped] Metadata fetched successfully from ${instance}`)
      sendResponse({ success: true, data: metadata })
      return
    } catch (e) {
  ...
  sendResponse({ success: false, error: 'All Piped instances failed for metadata' })
}
...
async function handleFetchPipedTranscript(request, sendResponse) {
  ...
  for (let i = 0; i < Math.min(instances.length, 5); i++) {
    ...
    try {
      ...
      if (!metadataResponse.ok) {
        console.warn(`[Piped] Instance ${instance} returned HTTP ${metadataResponse.status}`)
        continue
      }
      ...
      let subtitle = data.subtitles.find(s => s.code === lang && !s.autoGenerated)
      if (!subtitle) {
        subtitle = data.subtitles.find(s => s.code === lang)
      }
      if (!subtitle) {
        console.log(`[Piped] Language '${lang}' not found, using first available: ${data.subtitles[0].code}`)
        subtitle = data.subtitles[0]
      }

      console.log(`[Piped] Selected subtitle: ${subtitle.name} (${subtitle.code})`)
      ...
      if (!subtitleResponse.ok) {
        console.warn(`[Piped] Failed to fetch subtitle: HTTP ${subtitleResponse.status}`)
        continue
      }
      ...
      if (segments.length > 0) {
        console.log(`[Piped] Transcript fetched successfully: ${segments.length} segments`)
        sendResponse({ success: true, data: segments })
        return
      }
    } catch (e) {
  ...
  sendResponse({ success: false, error: 'All Piped instances failed for transcript' })
}
...
async function getPipedInstances() {
  ...
  if (cachedPipedInstances && (now - pipedInstancesCacheTime) < PIPED_CACHE_DURATION) {
    console.log(`[Piped] Using cached instances (${cachedPipedInstances.length} instances)`)
  ...
  try {
    ...
    const response = await fetch('https://raw.githubusercontent.com/TeamPiped/Piped/master/README.md', {
      signal: AbortSignal.timeout(8000)
    })
