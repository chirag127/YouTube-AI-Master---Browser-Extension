export const prompts = {
    summary: (t, o) => {
        ...
        switch (length) {
            case "Short":
                l =
                    "Provide a short summary with bullet points (approx. 100 words).";
                break;
            case "Detailed":
                l =
                    "Provide a comprehensive, detailed summary (approx. 600 words).";
                break;
            default:
                l =
                    "Provide a standard summary with paragraphs (approx. 300 words).";
        }
        ...
        if (metadata) {
            ...
            if (metadata.author)
                metadataContext += `Channel: ${metadata.author}\n`;
            if (metadata.description)
                metadataContext += `Description: ${metadata.description}\n`;
            if (metadata.category)
                metadataContext += `Category: ${metadata.category}\n`;
            if (metadata.keywords?.length)
                metadataContext += `Keywords: ${metadata.keywords.join(
                    ", "
                )}\n`;
            metadataContext += "\n";
        }
    ...
    comprehensive: (t, o) => {
        ...
        switch (length) {
            case "Short":
                l = "Short summary with bullet points (approx. 100 words)";
                break;
            case "Detailed":
                l = "Comprehensive, detailed summary (approx. 600 words)";
                break;
            default:
                l = "Standard summary with paragraphs (approx. 300 words)";
        }
...
2. Unpaid/Self Promotion - Self-promotion (merch, patreon) or unpaid shoutouts
3. Exclusive Access - Showcasing free/subsidized products/locations
4. Interaction Reminder (Subscribe) - "Like, Subscribe, Hit the Bell" requests
5. Highlight - Core value moment of video (ONLY ONE PER VIDEO)
6. Intermission/Intro Animation - Pauses, static frames, repeating animations
7. Endcards/Credits - Credits or YouTube endcards
8. Preview/Recap - "Coming up" teasers or "Previously on" recaps
9. Hook/Greetings - Generic intros/outros
10. Tangents/Jokes - Off-topic or entertainment filler
...
- The transcript contains hints like [Hint: Possible Sponsor]. Use these as suggestions but verify with your own understanding. You are the final decision maker.
