...
export async function sendChatMessage() {
  const i = document.getElementById('yt-ai-chat-input'),
    q = i?.value?.trim();
  if (!q) return;
  ...
  try {
    ...
    const metadata = state.currentVideoId
      ? await metadataExtractor.extract(state.currentVideoId)
      : null;
